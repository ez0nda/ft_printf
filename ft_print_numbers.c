...
/*   Updated: 2019/01/25 22:45:33 by ezonda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_indic_di2(t_struct *stru, t_stock *stock)
{
	...
	if (stru->flag[3] == 1 || stock->stock_il < 0)
		stru->flag[4] = 0;
	if (stru->prec > ft_nbrsize(ft_abs(stock->stock_il)))
	{
		i = stru->prec;
		stru->stock_pre2 = i - ft_nbrsize(ft_abs(stock->stock_il));
	}
	else
...
void	ft_indic_di(t_struct *stru, t_stock *stock)
{
	...
	if ((stock->stock_il < 0 && stru->prec >
		ft_nbrsize(ft_abs(stock->stock_il))) || (stru->flag[3] == 1
		&& stock->stock_il >= 0))
		i++;
	if (i > stru->min_field)
		stru->nb_carac = i;
...
{
	stru->stock_pre_f = stru->prec - ft_nbrsize(ft_abs(stock->stock_il));
	if (stru->flag[3] == 1 && stock->stock_il >= 0)
	{
		ft_putchar('+', stru);
		stru->nb_carac--;
